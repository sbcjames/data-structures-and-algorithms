...
const createServer = () => {
  ...
  app.get('/hello', function (req, res) {
    res.send('Hello hello! Winter is coming!');
  });
  app.get('/aboutme', function (req, res) {
    res.send('I am a software developer that watches Game of Thrones');
  });
  app.get('/favoritefoods', function (req, res) {
    res.send(['pizza', 'salad', 'cheese', 'chocolate']);
  });
...
const count = (target, input) => {
  return input.reduce((answerSoFar, valueOne) => {
    let count = valueOne.reduce((accumulator, valueTwo) => {
      if (valueTwo === target) {
        return accumulator + 1;
      }
      return accumulator;
    }, 0);
    return answerSoFar + count;
  }, 0);
};
...
const totalSum = (input) => {
  return input.reduce((answerSoFar, sumOne) => {
